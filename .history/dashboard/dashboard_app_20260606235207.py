...
def get_investment_grade(row, clf, scaler):
    ...
    buy_conf = confidence.get('BUY', 0)
    if buy_conf >= 40:
        return 'High', buy_conf
    elif buy_conf >= 25:
        return 'Medium', buy_conf
    else:
        return 'Low', buy_conf
        ...
        for t in tickers_all:
            ...
            if not df_t.empty:
                ...
                results.append({
                    ...
                    'RSI': f"{row['rsi']:.1f}",
                    'Signal': signal,
                    'BUY %': f"{confidence.get('BUY',0)}%",
                    'HOLD %': f"{confidence.get('HOLD',0)}%",
                    'SELL %': f"{confidence.get('SELL',0)}%",
                    'Grade': grade,
